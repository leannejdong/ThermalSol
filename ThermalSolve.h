...
namespace ThermalSol{
    void ThermalSolve(){
        std::cerr << LoadFile("data/Network_information.csv").value_or("File could not be opened!") << "\n";
        ifstream input("data/Network_information.csv");
        ...
        MatrixXi in_node = MatrixXd::Zero(168, m);
        MatrixXi out_node = MatrixXd::Zero(168, m);
        in_node.rowwise() = in;
        out_node.rowwise() = out;
